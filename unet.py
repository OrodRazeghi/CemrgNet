...
from tf_unet.layers import (weight_variable, weight_variable_devonc, bias_variable, conv2d, deconv2d, max_pool, crop_and_concat, pixel_wise_softmax, cross_entropy)
...
class Unet(object):
    ...
    def fastPredict(self, sess, x_test):
        y_dummy = np.empty((x_test.shape[0], x_test.shape[1], x_test.shape[2], self.n_class))
        prediction = sess.run(self.predicter, feed_dict={self.x: x_test, self.y: y_dummy, self.keep_prob: 1.})        
        return prediction
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, dropout=0.75, display_step=1,
              restore=False, write_graph=False, prediction_path='Data/Epochs'):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch * training_iters), ((epoch + 1) * training_iters)):
                    ...
                    #util.save_image(img, "%s/%s.jpg" % ("/data/or15/AtrialSegmentAuto/TechMultiDeep/inputs", nme))
                    ...
                    total_loss += loss
                    if lowest_loss > abs(total_loss):
                        lowest_loss = abs(total_loss)
                        early_stop = 0
    ...
    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y):
        ...
        logging.info(
            "Iter {:}, Minibatch Loss= {:.4f}, Training Accuracy= {:.4f}, Minibatch error= {:.1f}%".format(step, loss, acc, error_rate(predictions, batch_y)))
